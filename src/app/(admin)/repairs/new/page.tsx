...
import { getDeviceStepTwoAction } from "@/features/repairs/reception/actions";
...
export default async function NewRepairPage({
  ...
  try {
    ...
    if (deviceId) {
      const result = await getDeviceStepTwoAction(deviceId);
    ...
    return (
      <div className="space-y-6">
        <PageHeader
          eyebrow="Nueva recepción"
          title="Recibir equipo"
          description="Identificá al cliente, documentá el dispositivo y conservá evidencia objetiva de cómo ingresó al taller."
        />
        <Card className="mx-auto max-w-2xl p-6 text-center">
          <h2 className="text-lg font-bold text-primary">
            No pudimos preparar el formulario de recepción
          </h2>
          <p className="mt-2 text-sm leading-6 text-muted">
            La información necesaria no está disponible temporalmente. Volvé a
            intentarlo en unos instantes.
          </p>
          <ButtonLink className="mt-5" href="/repairs">
