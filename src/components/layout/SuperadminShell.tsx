...
const navigation = [
  ...
  { href: "/superadmin#system", label: "Sistema", icon: Settings },
] as const;
    ...
    <div className="app-shell app-background min-h-screen">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col bg-app-sidebar text-app-sidebar-text lg:flex">
        ...
        <div className="flex-1 px-4 py-5">
          ...
          <nav aria-label="Navegación de superadministración" className="space-y-1">
            {navigation.map((item, index) => { const Icon = item.icon; return <a className={`flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold transition-colors ${index === 0 ? "bg-app-sidebar-active text-app-sidebar-text" : "text-app-sidebar-muted hover:bg-white/8 hover:text-white"}`} href={item.href} key={item.href}><Icon className="size-[18px]" />{item.label}</a>; })}
          </nav>
        </div>
      ...
      <div className="min-h-screen lg:pl-64">
        <header className="sticky top-0 z-30 border-b border-app-border bg-app-surface/95 backdrop-blur lg:hidden">
          <div className="flex min-h-16 items-center justify-between gap-3 px-4"><AppLogo compact className="text-primary" /><div className="text-right"><strong className="block max-w-36 truncate text-xs text-primary">{name}</strong><span className="text-[11px] text-muted">Superadministrador</span></div></div>
          <nav aria-label="Navegación de superadministración" className="flex overflow-x-auto border-t border-border px-2">
            {navigation.map((item) => { const Icon = item.icon; return <a className="flex min-h-12 shrink-0 items-center gap-2 px-3 text-xs font-semibold text-muted hover:text-accent" href={item.href} key={item.href}><Icon className="size-4" />{item.label}</a>; })}
          </nav>
        </header>
