...
function stockState(item: InventoryItem) {
  if (item.currentStock === 0) return { label: "Sin stock", className: "bg-danger-soft text-danger ring-danger/20" };
  if (item.currentStock <= item.minimumStock) return { label: "Stock bajo", className: "bg-warning-soft text-warning ring-warning/20" };
  return { label: "Disponible", className: "bg-success-soft text-success ring-success/20" };
}
...
export function InventoryExplorer({ items }: { items: InventoryItem[] }) {
  ...
  return <div className="space-y-5"><Card className="p-4"><div className="grid gap-3 lg:grid-cols-[minmax(260px,1fr)_210px_auto_auto]"><label className="relative block"><span className="sr-only">Buscar inventario</span><Search className="pointer-events-none absolute left-3 top-1/2 size-5 -translate-y-1/2 text-muted" /><input className="field-control pl-11" onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Producto, código, marca o compatibilidad" type="search" value={query} /></label><select aria-label="Filtrar por categoría" className="field-control" disabled={!categories.length} onChange={(event) => { setCategory(event.target.value); setPage(1); }} value={category}><option value="ALL">Todas las categorías</option>{categories.map((name) => <option key={name} value={name}>{name}</option>)}</select><label className="flex min-h-11 items-center gap-3 rounded-lg border border-line bg-surface-raised px-4 text-sm font-semibold text-primary"><input checked={onlyAlerts} className="size-4 accent-accent" onChange={(event) => { setOnlyAlerts(event.target.checked); setPage(1); }} type="checkbox" />Sólo alertas</label><Button disabled={!query && category === "ALL" && !onlyAlerts} onClick={clearFilters} variant="secondary"><X className="size-4" />Limpiar</Button></div><p aria-live="polite" className="mt-3 text-xs font-semibold text-muted">{filtered.length} artículos encontrados</p></Card>{visibleItems.length ? <><Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead><tr className="border-b border-line bg-surface-soft text-xs font-bold uppercase tracking-wide text-muted"><th className="px-5 py-3">Producto</th><th className="px-5 py-3">Categoría</th><th className="px-5 py-3">Stock</th><th className="px-5 py-3">Mínimo</th><th className="px-5 py-3">Precio</th><th className="px-5 py-3">Estado</th></tr></thead><tbody>{visibleItems.map((item) => { const state = stockState(item); return <tr className="border-b border-line last:border-0" key={item.id}><td className="px-5 py-4"><strong className="block text-sm text-primary">{item.name}</strong><span className="text-xs text-muted">{item.code}{item.compatibility ? ` · ${item.compatibility}` : ""}</span></td><td className="px-5 py-4 text-sm text-muted">{item.category}</td><td className="px-5 py-4 text-lg font-bold text-primary">{item.currentStock}</td><td className="px-5 py-4 text-sm text-muted">{item.minimumStock}</td><td className="px-5 py-4 text-sm font-bold text-primary">{formatCurrency(item.price)}</td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ring-1 ring-inset ${state.className}`}>{state.label}</span></td></tr>; })}</tbody></table></div></Card><Pagination onPageChange={setPage} page={safePage} pageSize={PAGE_SIZE} totalItems={filtered.length} /></> : <Card><EmptyState title="No hay artículos para mostrar" description={items.length ? "Modificá la búsqueda o desactivá alguno de los filtros." : "Todavía no hay artículos registrados para esta organización."} /></Card>}</div>;
}
