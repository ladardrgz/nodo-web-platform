...
const PAGE_SIZE = 10;
export function RepairsExplorer({ repairs, referenceNow, statuses }: { repairs: ServiceOrderListItem[]; referenceNow: string; statuses: Array<{ code: string; name: string }> }) {
  ...
  const filteredRepairs = useMemo(() => {
    ...
    return repairs.filter((repair) => {
      const matchesQuery = !normalized || [repair.orderNumber, repair.customerName, repair.device.brand, repair.device.model, repair.reportedProblem].some((value) => String(value).toLocaleLowerCase("es").includes(normalized));
      const matchesStatus = status === "ALL" || repair.status.code === status;
